...
model = SpGAT(nb_classes=nb_classes, nb_nodes = features.shape[0], n_heads=n_heads, hid_units=hid_units, activation=nonlinearity,
                 ffd_drop=ffd_drop, attn_drop=attn_drop, residual=False)
...
for epoch in range(nb_epochs):
    ...
    print('Epoch: %d,  Training: loss = %.5f, acc = %.5f | Val: loss = %.5f, acc = %.5f, best,%.5f' %
    ...
    if val_acc >= vacc_mx:
        ...
        test_output, best_test_acc, ts_loss, _ = model.call(
            (features_tensor, biases_tensor, y_test_tensor, test_mask_tensor), training=False)
...
print('Test loss:', ts_loss.numpy(), '; Test accuracy:', best_test_acc.numpy())
