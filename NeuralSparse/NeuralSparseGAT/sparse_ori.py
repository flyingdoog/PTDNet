...
if sparse:
    ...
    if nb_noising_edges > 0:
        noise_row = np.random.choice(range(nb_nodes), nb_noising_edges)
        noise_col = np.random.choice(range(nb_nodes), nb_noising_edges)
        noise_data = np.ones_like(noise_row)
        noise_adj = coo_matrix((noise_data, (noise_row, noise_col)), shape=adj.shape)
        nadj = adj + noise_adj
        biases = process.preprocess_adj_bias(nadj)
    else:
